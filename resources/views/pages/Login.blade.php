...
<div class="min-w-screen min-h-screen bg-gray-200 flex items-center justify-center">
    <div class="bg-white shadow-md shadow-slate-300 py-4 px-5 rounded-md">
           @if(count($errors) === 1) 
            ...
            @endif
        <h2 class="text-2xl text-center font-bold">Login TokoSMK</h2>
        <form action="/login" method="POST" class="w-full mt-6">
            @csrf
            ...
            <button type="submit" class="mt-7 bg-blue-400 text-white font-semibold text-sm rounded-md w-full py-2">Login</button>
            <p class="text-slate-400 text-sm font-medium mt-3 text-center">
                Don't have account? <a href="/register" class="text-slate-500">Register</a>
            </p>
        </form>
    </div>
</div>
